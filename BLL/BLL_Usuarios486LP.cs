...
namespace BLL
{
    public class BLL_Usuarios486LP
    {
            ...
            try
            {
                ...
                if (hashIngresado != u.Contraseña)
                {
                    ...
                    if (intentos >= 3)
                    {
                        ...
                        ObjBitacora.Registrar(new BitacoraEvento486LP("Login", $"Usuario {nombreUsuario} bloqueado por 3 intentos fallidos.", "Muy Alto", u.DNI, u.NombreUsuario));
                        Mensaje = "Usuario bloqueado por 3 intentos fallidos. Contacte al administrador.";
                        return -3;
                    }

                    ObjBitacora.Registrar(new BitacoraEvento486LP("Login", $"Intento fallido {intentos}/3 para {nombreUsuario}.", "Muy Alto", u.DNI, u.NombreUsuario));
                ...
                ObjBitacora.Registrar(new BitacoraEvento486LP("Login", $"Inicio de sesión exitoso: {nombreUsuario}.", "Bajo", u.DNI, u.NombreUsuario));
                usuarioLogueado = u;
                return 1;
            }
            catch (Exception ex)
            {
                ObjBitacora.Registrar(new BitacoraEvento486LP("Login", $"Error en BLL_Usuarios.Login(): {ex.Message}", "Muy Alto", "Sistema", "Sistema"));
        ...
        public List<Usuario486LP> Listar()
        {
            ...
            {
                ObjBitacora.Registrar(new BitacoraEvento486LP("Gestión Usuarios", $"Error en BLL_Usuarios.Listar(): {ex.Message}", "Muy Alto", "Sistema"));
        ...
        public List<Usuario486LP> ListarActivos()
        {
            ...
            {
                ObjBitacora.Registrar(new BitacoraEvento486LP("Gestión Usuarios", $"Error en BLL_Usuarios.ListarActivos(): {ex.Message}", "Muy Alto", "Sistema"));
                return new List<Usuario486LP>();
            }
        }
        ...
        public bool BloquearPorDNI(string dni, out string Mensaje)
        {
            try
            {
                ...
                if (resultado)
                {
                    ObjBitacora.Registrar(new BitacoraEvento486LP("Gestión Usuarios", $"Usuario bloqueado manualmente. DNI: {dni}.", "Muy Alto", SessionManager486LP.ObtenerInstancia().UsuarioActual()?.DNI ?? "Sistema"));
                }
            ...
            {
                ObjBitacora.Registrar(new BitacoraEvento486LP("Gestión Usuarios", $"Error en BLL_Usuarios.BloquearPorDNI(): {ex.Message}", "Muy Alto", "Sistema"));
